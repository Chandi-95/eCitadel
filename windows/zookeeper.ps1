...
while($resp.Key -notlike "Enter"){while (-not [console]::KeyAvailable) {Start-Sleep -Milliseconds 50} $resp = [console]::ReadKey($true)} $resp = "CTRL"
Write-Host ""
...
if ($IsDC) {
foreach($group in Get-ADUser | Where-Object Name -notlike "Domain Users"){
foreach($member in Get-ADGroupMember -Group $group.Name){
$prompt = "[Instruction] Should user " + $Member.Name + " be in " + $Group.Name + ": Y/n "
$yn = read-host $prompt

if($yn -eq "n" -or $yn -eq "N"){
Remove-ADGroupMember -Group $Group -Member $member
